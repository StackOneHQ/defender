...
import { normalizeUnicode, normalizeWhitespace, stripCombiningMarks } from "../src/sanitizers/normalizer";
...
describe("normalizeUnicode (returned-output safe)", () => {
	it("preserves precomposed accents (NFKC keeps them)", () => {
		// Accented text must NOT be rewritten to ASCII — normalizeUnicode does
		// compatibility normalization only, so benign accented content survives.
