...
describe.skipIf(!!process.env.CI)("coldLoad — cold-start telemetry", () => {
	it("coldLoad === (model was not ready before the call); warm afterwards", async () => {
		...
		const first = await defense.defendToolResult(
			{ content: "first call with enough text to classify" },
			"docs_get",
		);
		...
		const second = await defense.defendToolResult(
			{ content: "second call with enough text to classify" },
			"docs_get",
		);
