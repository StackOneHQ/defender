...
export class OnnxClassifier {
  ...
  async classify(text: string): Promise<number> {
    ...
    const inputIdsTensor = new this.OrtTensor!(
      'int64',
      inputIds,
      [1, inputIds.length]
    );
    const attentionMaskTensor = new this.OrtTensor!(
      'int64',
      attentionMask,
  ...
  async classifyBatch(texts: string[]): Promise<number[]> {
    ...
    for (let i = 0; i < batchSize; i++) {
      ...
      for (let j = 0; j < t.inputIds.length; j++) {
        batchInputIds[i * maxLen + j] = t.inputIds[j]!;
        batchAttentionMask[i * maxLen + j] = t.attentionMask[j]!;
      }
    ...
    const scores: number[] = [];
    for (let i = 0; i < batchSize; i++) {
      scores.push(sigmoid(Number(logits.data[i])));
    }
  ...
  private tokenize(text: string): {
    ...
    const rawMask: bigint[] = Array.isArray(encoded.attention_mask)
      ...
      : (encoded.attention_mask as { tolist: () => bigint[][] })
          .tolist()
          .flat();
